...
if(isset($_POST['option'])){
	...
	if($_POST['option'] == "all"){
		$sqlMyOrder = "SELECT * FROM purchase WHERE customer = '".$username."' ORDER BY receiptID DESC";
		$result = $conn->query($sqlMyOrder);

	}else if($_POST['option'] == "inprocess"){
		$sqlMyOrder = "SELECT * FROM purchase WHERE customer = '".$username."' AND status = 'in process' ORDER BY receiptID DESC";
		$result = $conn->query($sqlMyOrder);
	}else if($_POST['option'] == "delivered"){

		$sqlMyOrder = "SELECT * FROM purchase WHERE customer = '".$username."' AND status = 'delivered' ORDER BY receiptID DESC";
		$result = $conn->query($sqlMyOrder);
	}
				...
				<div class="order-info">
					<div class="pull-left">
						...
						<div class="pull-cont" >
								...
								<a href="order.php?receiptID=<?php echo $row['receiptID'] ?>" class="pull-rightmanage" name="submit">View</a>
								<div class="clear">
